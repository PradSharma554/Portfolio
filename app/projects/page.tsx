...
const projects = [
  {
    ...
    description: "A blockchain-based parcel tracker ensuring transparency and real-time updates. Uses decentralized ledgers to prevent fraud and enable audit trails.",
          ...
          <div ref={triggerRef}>
            <div ref={sectionRef} className="h-screen flex flex-row relative w-max">
              ...
              <div className="w-screen h-full flex flex-col justify-center px-10 md:px-20 bg-black border-r border-white/10 z-10 shrink-0">
                <button
                  onClick={() => router.push('/')}
                  className="flex items-center gap-2 text-purple-400 hover:text-purple-300 transition-colors mb-10 group"
                >
                     ...
                     <div key={project.id} className="relative group w-[400px] md:w-[600px] aspect-[4/3] bg-zinc-900 rounded-lg overflow-hidden border border-white/5 hover:border-purple-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-purple-500/20">
                       ...
                       <div className="absolute inset-0 p-10 flex flex-col justify-between">
                         <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                           <span className="text-xs font-mono text-purple-300 uppercase tracking-widest border border-purple-500/30 px-2 py-1 rounded">{project.category}</span>
                           <h3 className="text-4xl font-bold font-space text-white mt-4">{project.title}</h3>
                         </div>
                         ...
                         <div className="opacity-0 group-hover:opacity-100 translate-y-10 group-hover:translate-y-0 transition-all duration-500 delay-100">
                           ...
                           <div className="flex flex-wrap gap-2 mb-8">
                             {project.stack.map(tech => (
                               <span key={tech} className="text-xs text-gray-500 bg-black/50 px-2 py-1 rounded backdrop-blur">{tech}</span>
                             ))}
                           </div>

                           <div className="flex gap-4">
                             <a href={project.github} className="p-3 bg-white/10 rounded-full hover:bg-white text-black hover:scale-110 transition-all">
                               <Github className="w-5 h-5 text-white hover:text-black transition-colors" />
                             </a>
                             <a href={project.link} className="p-3 bg-purple-600 rounded-full hover:bg-purple-500 hover:scale-110 transition-all">
